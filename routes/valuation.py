    ...
    try:
        ...
        if not valuation_service:
            ...
            return ErrorHandler.api_error(
                "Valuation service not available. Please ensure GEMINI_API_KEY is configured."
            )
        ...
        try:
            ...
            logger.info(
                f"Received financial data keys: {list(financial_data.keys()) if financial_data else None}"
            )
        ...
        missing_sections = [
            section for section in required_sections if section not in financial_data
        ]

        if missing_sections:
            logger.error(f"Missing required sections: {missing_sections}")
            return ErrorHandler.validation_error(
                f"Missing required financial data sections: {', '.join(missing_sections)}"
            )
        ...
        if valuation_result.get("success"):
            ...
            return ErrorHandler.processing_error(
                valuation_result.get("error", "Valuation analysis failed")
            )
