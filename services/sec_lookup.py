...
class SECLookupService:
    ...
    def lookup_company(self, company_name: str, threshold: int = 85) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fuzzy search with multiple scoring methods for better suggestions
            ...
            # For queries longer than 3 characters, use Gemini-assisted resolution instead of fuzzy matching
            # This prevents wrong matches like "Google" -> "G" (Genpact)
            if len(query) >= 4:
                try:
                    from .gemini_service import GeminiFinancialExtractor
                    gemini = GeminiFinancialExtractor()
                    
                    # Get company names for Gemini
                    ...
                    if gemini_result["success"] and gemini_result["suggestions"]:
                        # If Gemini finds a high-confidence match, try to look it up
                        top_suggestion = gemini_result["suggestions"][0]
                        if top_suggestion["confidence"] >= 85:
                            # Try to find this company in our database
                        ...
                        # Return Gemini suggestions for user selection
                        gemini_suggestions = []
            ...
            # Use fuzzy matching for short queries or when Gemini fails
            if best_score >= threshold:
                # Auto-accept good match
            ...
            # Use fuzzy matching results as fallback
